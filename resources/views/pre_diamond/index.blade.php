        ...
        <div class="row">
            <div class="card">
                <div class="card-header">
                    <div class="" style="display: flex; justify-content: space-between;"><h3 >PREMIUM DIAMONDS</h3>
                </div>
                ...
                <div class="card-body">
                    ...
                    <tbody id="myTable">
                    <?php foreach ($pre_diamonds as $pre_diamond){?>
                    
                        <tr>
                            
                            <td>{{ $pre_diamond->date }}</td>
                            <td>{{ $pre_diamond->request_by }}</td>
                            <td>{{ $pre_diamond->name }}</td>
                            <td>{{ $pre_diamond->order }}</td>
                            <td>{{ $pre_diamond->diamonds }}</td>
                            <td>{{ $pre_diamond->coins }}</td>
                            <td>{{ $pre_diamond->ml_id }}</td>
                            <td>{{ $pre_diamond->ign }}</td>
                            <td>{{ $pre_diamond->ref }}</td>
                            <td style="
                            <?php 
                                if($pre_diamond->status == 'Pending'){echo 'background-color: #ff0000; color: white;';}
                                if($pre_diamond->status == 'Done'){echo 'background-color: green; color: white;';}
                                if($pre_diamond->status == 'Cancel'){echo 'background-color: black; color: white;';}
                            ?>">
                                {{ $pre_diamond->status }}</td>
                            <td>{{ $pre_diamond->payment_method }}</td>
                            <td>{{ $pre_diamond->profit }}</td>
                            <td style="background-color: white"><a href="/premium_diamonds/<?= $pre_diamond->id?>" class="btn btn-primary ">Edit</a></td>
                        </tr>
                    <?php }?>
                    ...
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            ...
                            <div class="form-group">
                                <form  action="{{ route('pre_diamond.store') }}" method="post" id="orderForm">
                                @csrf
