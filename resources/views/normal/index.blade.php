            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <?php foreach ($normals as $normal){?>
                        <tr>
                            <td>{{ $normal->date }}</td>
                            <td>{{ $normal->request_by }}</td>
                            <td>{{ $normal->name }}</td>
                            <td>{{ $normal->name_of_skin }}</td>
                            <td>{{ $normal->type_of_skin }}</td>
                            <td style="
                            <?php 
                                if($normal->status == 'Pending'){echo 'background-color: #ff0000; color: white;';}
                                if($normal->status == 'Done'){echo 'background-color: green; color: white;';}
                                if($normal->status == 'Cancel'){echo 'background-color: black; color: white;';}
                            ?>">
                                {{ $normal->status }}</td>
                            <td>{{ $normal->ml_to_follow }}</td>
                            <td style="
                            <?php 
                                if($normal->follow_status == 'Pending'){echo 'background-color: #ff0000; color: white;';}
                                if($normal->follow_status == 'Done'){echo 'background-color: green; color: white;';}
                                if($normal->follow_status == 'Cancel'){echo 'background-color: black; color: white;';}
                            ?>">
                                {{ $normal->follow_status }}</td>
                            <td>{{ $normal->Schedule }}</td>
                            <td>{{ $normal->payment }}</td>
                            <td>{{ $normal->ref }}</td>
                            <td>{{ $normal->payment_method }}</td>
                            <td>{{ $normal->profit }}</td>
                            <td style="background-color: white"><a href="/normal_gifting/<?= $normal->id?>" class="btn btn-primary ">Edit</a></td>
                        </tr>
                    <?php }?>
                    ...
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            ...
                            <div class="form-group">
                                <form  action="{{ route('normal.store') }}" method="post" id="orderForm">
                                @csrf
